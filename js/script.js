...
try{
  ...
  // Safety net: if nothing became visible after 700ms on touch, show all
  if(isTouch){
    setTimeout(()=>{
      const anyVisible = Array.from(document.querySelectorAll('.reveal')).some(el=> el.classList.contains('visible'));
...
// Lighter particle density on small screens
function particlesScale(){
  return (Math.min(window.innerWidth, window.innerHeight) < 640) ? 0.6 : 1;
}
const _createParticles = createParticles;
createParticles = function(){
  _createParticles();
  if(particles){ particles = particles.slice(0, Math.floor(particles.length * particlesScale())); }
};
resize();
